window.onload = function() {
    ...
    function QingLv(config) {
        ...
        this.createQL = function () {
            ...
            if (e) {
                ...
                e.events.onInputDown.add(this.handleMultipleTouch, this);
            }
        };
        this.hitted = function (sprite) {
            score += 1000; // Увеличиваем счет на 1000 при каждом попадании
        ...
        this.handleMultipleTouch = function (sprite, pointer) {
            // Дополнительная обработка мультитач
            console.log('Multitouch event: ', pointer);
        };
    }
    ...
    game.States.preload = function() {
        this.preload = function() {
            ...
            game.load.spritesheet('daojishi', '/static/assets/img/daojishi.png', 200, 200, 3);
            game.load.image('bg', '/static/assets/img/bg.png');
            game.load.spritesheet('qinglvdog', '/static/assets/img/qinglvdog.png', 80, 77, 2);
            game.load.image('dog', '/static/assets/img/dog.png');
            // game.load.image('logo', '/static/assets/img/logo.png');
            game.load.image('shengyu', '/static/assets/img/shengyu.png');
            game.load.image('chaisan', '/static/assets/img/chaisan.png');
        };
        this.create = function() {
            console.log('Assets preloaded, starting main state');
    ...
    game.States.main = function() {
        this.create = function() {
            console.log('Main state started');
            game.input.maxPointers = 10; // Активация мульти-тач
            // 剩余时间
            this.leftTime = time;
        ...
        this.refreshTime = function() {
            ...
            if (this.leftTime === 0) {
                ...
                // Сохранение времени окончания игры

                // Перевод баланса в базу данных
              fetch('/update_balance', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json'
    },
    body: JSON.stringify({ user_id: userId, balance: score, game_type: 'SCRATS' })
}).then(response => response.json())
  .then(data => {
      console.log('Баланс обновлен:', data);
      // Обнуление счёта после отправки баланса
      score = 0;
...
// Предотвращение стандартных действий на касание
document.addEventListener('touchstart', function(event) {
    event.preventDefault(); // Предотвращает стандартное поведение касания
}, { passive: false });

document.addEventListener('touchmove', function(event) {
    event.preventDefault(); // Предотвращает прокрутку при движении
}, { passive: false });
